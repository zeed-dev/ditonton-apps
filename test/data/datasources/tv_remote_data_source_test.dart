...
void main() {
  ...
  group('get tv season', () {
    final tTvSeason = SeasonResponse.fromJson(
    ...
    test('should return list of Tv Model when the response code is 200',
        () async {
      // arrange
      when(mockHttpClient.get(Uri.parse('$BASE_URL/tv/$tId/season/1?$API_KEY')))
          .thenAnswer((_) async =>
              http.Response(readJson('dummy_data/tv_season.json'), 200));
      // act
      final result = await dataSource.getTvSeason(tId);
      // assert
    ...
    test('should throw Server Exception when the response code is 404 or other',
      ...
      when(mockHttpClient.get(Uri.parse('$BASE_URL/tv/$tId/season/1?$API_KEY')))
          .thenAnswer((_) async => http.Response('Not Found', 404));
      // act
      final call = dataSource.getTvSeason(tId);
      // assert
      expect(() => call, throwsA(isA<ServerException>()));
    });
  });
